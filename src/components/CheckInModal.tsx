...
import { PPE_ASSETS } from '@/data/assets'
    ...
    return (
      <Modal open onClose={onClose} size="md">
        <div className="animate-in py-4 flex flex-col items-center text-center gap-5">
          ...
          >
            <i className="ri-checkbox-circle-fill text-4xl" style={{ color: 'var(--color-badge-green-dark)' }} />
          </div>
          <div>
  ...
  return (
    ...
    >
      <div className="max-h-[65vh] overflow-y-auto space-y-8 -mx-5 px-5 pb-1">
        ...
        {/* Photo — only for pre-scheduled visits; walk-ins already have a photo from the form */}
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            {([true, false] as const).map((val) => (
              <button
                ...
                className={`rounded-xl border px-4 py-3 text-sm font-medium transition-colors ${issueAssets === val
                  ? val
                ...
                {PPE_ASSETS.map((asset) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center gap-1.5 rounded-xl border px-2 py-2.5 text-center transition-colors ${active
                        ? 'border-brand bg-brand/10 text-brand'
                        : 'border-border bg-surface text-text-secondary hover:bg-surface-secondary hover:text-text-primary'
                        }`}
                    >
        ...
        {/* Badge — last: it's the output of the check-in process, not an input */}
        <div>

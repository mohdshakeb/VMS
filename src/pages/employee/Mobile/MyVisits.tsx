...
export default function MyVisitsMobile() {
  ...
  const tabs: { label: string; value: FilterTab; count: number }[] = [
    ...
    { label: 'Done', value: 'completed', count: myVisits.filter((v) => (COMPLETED_STATUSES as string[]).includes(v.status)).length },
    { label: 'Cancelled', value: 'cancelled-rejected', count: myVisits.filter((v) => (CLOSED_STATUSES as string[]).includes(v.status)).length },
  ]

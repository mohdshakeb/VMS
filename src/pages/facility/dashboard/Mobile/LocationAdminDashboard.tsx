...
import type { FacilityComplianceStatus, FacilityType, ComplianceRecord } from '@/types/facility'
import Button from '@/components/Button'
import type { LocationGroup } from '@/utils/facilityHelpers'
...
import { LOCATION_ADMIN_LOCATIONS, groupFacilitiesByLocation } from '@/utils/facilityHelpers'
...
function SingleLocationCardMobile({
  group,
  record,
  onCompliance,
  onDetail,
  ...
  return (
    <div className="bg-white rounded-xl border border-border-light">
      {/* Header */}
      <div className="p-4 flex items-start gap-3 border-b border-border-light">
        <div className="h-12 w-12 rounded-full bg-brand-red-50 border border-brand-red-100 flex items-center justify-center shrink-0">
          <span className="text-sm font-semibold text-brand">{initials}</span>
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-text-primary leading-tight truncate">{group.location}</p>
          <p className="text-xs text-text-secondary mt-0.5">{firstFacility.city} · {firstFacility.state}</p>
        </div>
        {record && <FacilityStatusBadge status={record.status} />}
      </div>
...
function CompactLocationCardMobile({
  group,
  record,
  onCompliance,
  onDetail,
  ...
  return (
    <div className="bg-white rounded-xl border border-border-light">
      ...
      {/* Meta */}
      <div className="pl-[4.25rem] pr-4 pb-2 flex flex-wrap gap-x-3 gap-y-1 text-xs text-text-secondary">
      ...
      {/* Full-width buttons (VisitCard mobile pattern) */}
      <div className="px-4 pb-3 flex items-center gap-1.5">
        <Button size="sm" variant="secondary" fullWidth onClick={onDetail}>View Detail</Button>
        {ctaLabel && (
          <Button size="sm" variant="primary" fullWidth onClick={onCompliance}>{ctaLabel}</Button>
        )}
      </div>
    </div>
  )
...
export default function LocationAdminDashboardMobile() {
  ...
  const isSingle = locationGroups.length === 1
    ...
    <div className="md:hidden h-full flex flex-col bg-surface-secondary">
      <div className="flex-1 overflow-y-auto">
        <div className="px-4 pt-3 pb-10 flex flex-col gap-4">
          ...
          {/* Location cards */}
          {isSingle ? (
            <SingleLocationCardMobile
              group={locationGroups[0]}
              record={getRecord(locationGroups[0].location)}
              onCompliance={() => navigate(compliancePath(getRecord(locationGroups[0].location)))}
              onDetail={() => navigate(`/facility/locations/${encodeURIComponent(locationGroups[0].location)}`)}
            />
          ) : (
                      ...
                      <div key={notification.id}>
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (['compliance-submitted', 'sbu-edited', 'compliance-overdue', 'compliance-missed'].includes(notification.type)) {
                              navigate(notification.recordId ? `/facility/compliance/record/${notification.recordId}` : '/facility/compliance')
                            } else if (['compliance-window-open', 'compliance-deadline-reminder'].includes(notification.type)) {

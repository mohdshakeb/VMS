...
export default function MyFacilitiesDesktop() {
  ...
  return (
    <div className="hidden md:flex md:flex-col h-full bg-surface-secondary">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
        ...
        {filtered.length > 0 && (
          <p className="text-sm text-text-secondary">{filtered.length} facilit{filtered.length !== 1 ? 'ies' : 'y'}</p>
        )}
      </div>
    </div>
  )
...
// Re-export helpers used by the mobile twin to avoid duplication
export { isSubmitted, getLastCompliance, MONTH_SHORT, COMPLIANCE_LABEL, COMPLIANCE_STYLE, DUE_DEADLINE, NEXT_DEADLINE }
export type { }

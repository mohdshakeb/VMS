...
var getStrokeProperties = (borderStyle) => {
	switch (borderStyle) {
		case "dashed": return { strokeDasharray: "8 4" };
		case "dotted": return { strokeDasharray: "2 2" };
		case "dash-dot": return { strokeDasharray: "8 4 2 4" };
		case "dash-double-dot": return { strokeDasharray: "8 4 2 4 2 4" };
		default: return { strokeDasharray: "none" };
	}
};
var India = ({ type, size = constants.WIDTH, mapColor = constants.MAPCOLOR, strokeColor = constants.STROKE_COLOR, strokeWidth = constants.STROKE_WIDTH, hoverColor, selectColor, hints, hintTextColor, hintBackgroundColor, hintPadding, hintBorderRadius, onSelect, cityColors = {}, disableClick = false, disableHover = false, borderStyle }) => {
	if (type === "select-single") return /* @__PURE__ */ import_react.createElement(IndiaSingle, {
		type: "select-single",
		size,
		mapColor,
		strokeColor,
	...
	else if (type === "select-multiple") return /* @__PURE__ */ import_react.createElement(IndiaMultiple, {
...
var IndiaSingle = ({ size, mapColor, strokeColor, selectColor, strokeWidth, hoverColor, hints, onSelect, hintTextColor, hintBackgroundColor, hintPadding, hintBorderRadius, cityColors, disableClick, disableHover, borderStyle }) => {
	const instanceId = (0, import_react.useId)().replace(/:/g, "");
	const { x, y } = mouseTrack_default();
	const [stateHovered, setStateHovered] = (0, import_react.useState)(null);
	const [selectedState, setSelectedState] = (0, import_react.useState)(null);
	const [viewBox, setViewBox] = (0, import_react.useState)("0 0 100 100");
	const strokeProps = (0, import_react.useMemo)(() => getStrokeProperties(borderStyle), [borderStyle]);
	(0, import_react.useEffect)(() => {
	...
	const mapStyle = (0, import_react.useMemo)(() => ({
		width: size,
		stroke: strokeColor,
		strokeWidth,
		...strokeProps
	...
	(0, import_react.useEffect)(() => {
		stateCode.forEach((state) => {
			...
			if (path) path.style.fill = cityColors[state] || mapColor;
		});
	}, [
		cityColors,
		mapColor,
	...
	(0, import_react.useEffect)(() => {
		if (selectedState) {
			...
			if (path) path.style.fill = selectColor || constants.SELECTED_COLOR;
		}
	}, [
		selectedState,
		selectColor,
	...
	const handleMouseEnter = (hoverStateId) => {
		...
		if (path && !disableHover) path.style.fill = selectedState === hoverStateId ? selectColor || constants.SELECTED_COLOR : hoverColor || constants.HOVERCOLOR;
	};
	const handleMouseLeave = (hoverStateId) => {
		const path = document.getElementById(`${hoverStateId}-${instanceId}`);
		setStateHovered(null);
		if (path && !disableHover) path.style.fill = selectedState === hoverStateId ? selectColor || constants.SELECTED_COLOR : cityColors[hoverStateId] || mapColor;
	};
	const handleClick = (stateCode2) => {
		if (disableClick) return;
		if (selectedState === stateCode2) {
			const path = document.getElementById(`${stateCode2}-${instanceId}`);
			if (path) path.style.fill = cityColors[stateCode2] || mapColor;
			setSelectedState(null);
			if (onSelect) onSelect(null);
		} else {
			if (selectedState) {
				const previousPath = document.getElementById(`${selectedState}-${instanceId}`);
				if (previousPath) previousPath.style.fill = cityColors[selectedState] || mapColor;
			}
			setSelectedState(stateCode2);
			if (onSelect) onSelect(stateCode2);
		}
	};
	return /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, /* @__PURE__ */ import_react.createElement("div", {
		...
		y: "0px",
		viewBox
	}, stateCode?.map((code, index) => /* @__PURE__ */ import_react.createElement("path", {
		key: index,
	...
	})))), hints && stateHovered && /* @__PURE__ */ import_react.createElement("div", { style: hintStyle }, stateHovered));
};
var IndiaMultiple = ({ size, mapColor, strokeColor, selectColor, strokeWidth, hoverColor, hints, hintTextColor, hintBackgroundColor, hintPadding, hintBorderRadius, onSelect, cityColors, disableClick, disableHover, borderStyle }) => {
	const instanceId = (0, import_react.useId)().replace(/:/g, "");
	const { x, y } = mouseTrack_default();
	const [selectedStates, setSelectedStates] = (0, import_react.useState)([]);
	const [stateHovered, setStateHovered] = (0, import_react.useState)(null);
	const [viewBox, setViewBox] = (0, import_react.useState)("0 0 100 100");
	const strokeProps = (0, import_react.useMemo)(() => getStrokeProperties(borderStyle), [borderStyle]);
	(0, import_react.useEffect)(() => {
	...
	const handleMouseEnter = (hoverStateId) => {
		const path = document.getElementById(`${hoverStateId}-${instanceId}`);
		setStateHovered(hoverStateId);
		if (path && !disableHover) path.style.fill = selectedStates.includes(hoverStateId) ? selectColor || constants.SELECTED_COLOR : hoverColor || constants.HOVERCOLOR;
	};
	const handleMouseLeave = (hoverStateId) => {
		const path = document.getElementById(`${hoverStateId}-${instanceId}`);
		setStateHovered(null);
		if (path && !disableHover) path.style.fill = selectedStates.includes(hoverStateId) ? selectColor || constants.SELECTED_COLOR : cityColors[hoverStateId] || mapColor;
	};
	const handleClick = (stateCode2) => {
		if (disableClick) return;
		if (selectedStates.includes(stateCode2)) {
			...
			if (onSelect) onSelect(stateCode2, updatedSelectedStates);
		} else setSelectedStates((prevStates) => {
			const updatedStates = [...prevStates, stateCode2];
			const path = document.getElementById(`${stateCode2}-${instanceId}`);
			if (path) path.style.fill = selectColor || constants.SELECTED_COLOR;
			if (onSelect) onSelect(stateCode2, updatedStates);
...
var src_default = India;
//#endregion
export { src_default as default };

//# sourceMappingURL=@react-map_india.js.map